...
These objects were usually spurious detections or blends with bright stars,
found just outside our star masking radius (\footnote{Masa Yamamoto, private
communication}.  We did not keep enough information to exclude these objects
from our catalogs after the fact, and rerunning was prohibitive due to limited
computing resources.  As a temporary solution we used a simple ellipticity
dependent weight function \citep{ba14}
\begin{equation}
    W(|g|) = \left[1 - g^2\right]^2 \mathrm{exp}\left[ -e^2/2 \sigma^2\right]
\end{equation}
with $\sigma = 0.3$.  We found that using this weight function reduced the
uncertainty of the shear recovery by 13\% and removed a small ($m ~ 0.001$)
bias at low S/N.  In future work it will be important to use sufficient
star masking and identify any problematic unmasked objects.


\subsubsection{Dependence of Shear Bias on Stellar Density Cut} \label{sec:results:sdens}

Including these spurious or badly measured objects increased
the noise in the 
