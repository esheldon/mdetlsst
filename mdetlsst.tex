...
cuts on the measured signal to noise ratio.
...
find the lowest uncertainty for the case of maximum stellar density 100 per
...
with higher stellar density, we may want to exclude them for other reasons,
such as to reduce the impact of interstellar extinction or other sources of
error that correlate with stellar density.  It may be that we can tune such
data cuts to optimize a combination of accuracy and shear sensitivity.

We find that including detections with \snr\ as low 10 does not result less
uncertainty cutting at 12.5 or 15.  It may seem surprising that including more
objects can result in a noisier measurement.  This can occur with \mcal\
because the shear estimator involves dividing by the mean response, which is
typically smaller and noisier for lower \snr\ objects.  However, we have two
notes on this.  First, we did not use any explicit weighting of the
ellipticities when calculating the mean shear.   It may be that a smooth weight
that is a function of \snr, $T$, and other relevant parameters will provide a
better sensitivity \esstodo{cite Y3 paper weighting}.  Second, these results
will vary significantly with the measurement type, e.g.  the specific weight
function used or model fitted, which generally provide different \snr\
estimates.
