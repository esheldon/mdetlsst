...
image frame with the LSST camera pixel scale of 0.2 arcseconds (XXX reference).
...
the camera rotations used by LSST cam (XXX cite proper paper).  The images were
also shifted, or ``dithered'', randomly in two dimensions relative to a
reference sky position corresponding to the center of the desired final coadded
image.  We choose the dithers to be a unrealistically small, within two pixels,
in order to minimize wasted time simulating portions of the image that would
not overlap the final coadd.  The image size was large enough such that the
rotated image, after dithers, would have no edge crossing the coadd region,
again to avoid waste:  coadding an image with an edge produces a discontinuous
PSF in the final coadd, so such images would be discarded.
