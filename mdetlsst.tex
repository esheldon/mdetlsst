...
\begin{itemize}
    ...
    \item Short version:  We are excited to use the new data from Stage IV
        experiments such as Rubin LSST to study dark energy.  Lensing is one of
        the phenomena we will use to study dark energy.  We need to make sure
        we have codes ready, and that these codes can give accurate results
        with well characterized data.  The joint m(z)/N(z) calibration may
        require simulations even if the mean $m(z)$ is small, but the smaller
        the mean $m(z)$ the easier it is to constrain N(z).  \Mdet\ has been
        proven accurate enough, even with blending, in an idealistic scenario.
        Here we test \mdet\ with more realistic simulations, using an
        implementation that works with Rubin data.

\end{itemize}

\esstodo{More intro before this}

Weak gravitational lensing shear signals \esstodo{cite} can be measured
accurately from the lensed images of galaxies, up to non-linear shear effects,
in the presence of noise, a finite point spread function (PSF) and overlapping
galaxy images using the \mdet\ technique
\citep{mdet20,HoekstraMdet2021a,HoekstraMdet2021b}.

% The shear measurement
% problem is solved, in principle.
...
We think it is reasonable that the shear measurement implementation will
provide accurate measurements in the presence of these types of features.  In
other words, once the data are characterized in terms of flux and astrometric
calibration, background determination, and PSF, and problematic features are
identified and potentially masked, the shear code should be able to process the
data and to provide an accurate shear calibration.

We also expect the images may be combined into ``coadded'' aggregates
\esstodo{cite euclid, Roman, as well as LSST}, which typically have correlated
pixel noise due to the remapping, or ``warping'' to a common frame.  The
imminent Rubin Observatory Legacy Survey of Space and Time
...
requires coaddition for all but the most efficient measurements.  Again, we
think the shear code should perform accurately in the presence of correlated
noise, provided it is well characterized.

At the time of writing, we consider characterization of the data, such as
determining the flux and astrometric calibration, PSF characterization,
...
containing galaxies, stars with a realistic galactic density distribution, and
proxies for the above image artifacts, which we warp and aggregate into
overlapping coadds.  We simulate the expected data products produced by the
...
and algorithms\footnote{The software used in this work is freely available on the
internet.  URLs are provided as footnotes to the text.}.
 
\esstodo{Section break down?}
...
\citep{SheldonMcal2017,mdet20}.  We stored this noise image in a copy of the
image \calexp\ structure, but replacing the real image data with the noise
image.
