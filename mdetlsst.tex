...
in this configuration due only to second order shear effects (see \S
\ref{sec:results:secondorder}).

We then shear recovery with each additional feature separately in order to
isolate the cause of potential biases.  We found a bias only in the case of
extreme spatial PSF variation and few simulated epochs (see \S
\ref{sec:results:psfvar}).  We present results with all simulated features in
\S \ref{sec:results:cells}.

We assume a simple linear model \citep[see, e.g.,][]{heymans2006} for
estimating a bias in the recovered shear
\begin{equation} \label{eq:m}
\langle \vecg \rangle \equiv \vecc + (1 + \vecm) \vecgam
\end{equation}
...
\esstodo{Establish 2nd order shear effects with grid/simple.  Demonstrate PS
