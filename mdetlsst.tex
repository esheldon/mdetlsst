...
the presence of these types of features.  We can define an accurate shear
measurement algorithm as one that is unbiased for noisy, blended, but well
characterized images.  In other words, if the data are well characterized in
terms of flux calibration, astrometry, background determination, noise
properties, and PSF, and if problematic features are identified and masked, the
shear code should be able to provide an accurate shear measurement.

Errors in the low-level characterization of the data may contribute
significantly to the systematic error for an actual analysis on real data.  For
an unbiased shear measurement algorithm, the limiting factor would then be the
...
``coadded'', or summed images, which, for standard implementations, have
correlated pixel noise due to the remapping, or ``warping'', of each image to a
common frame before combination. LSST will image the survey area hundreds of
times, producing a large data volume, the processing of which requires
coaddition for all but the most efficient measurements.
...
We also record the moments of the coadd PSF, after reconvolution, for use in
object selection (see \S \ref{sec:results:full}).
...
corresponding value for the PSF after reconvolution. The lower \snr\ cut
...
shear recovery.  As show in \citep{BeckerMdetCoadd}, we find that the shear can
