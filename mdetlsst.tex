...
We use weighted moments for object measurement, with a fixed, circular, Gaussian
weight function $G(x, y)$ with full width at half maximum of 1.2 arcseconds.  We record
the flux, S/N, second moments, and ellipticity derived from the second moments:
\begin{eqnarray}
    F &=& \sum G(x, y) I(x, y) \\
...
pixel by its inverse variance; because the noise is constant across the coadd
this only has an effect for masked, zero weight pixels.
