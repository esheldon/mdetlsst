...
atmospheric and optics simulations.  Its parameters were set to generate the
...
Note the noise used for this process should include only the background noise
estimate, not an estimate of the Poisson noise from the objects, which in real
images would be sheared and is considered part of the lensing signal.
...
any sub-pixel offsets between the pixel grids, so the PSF images were not
centered.  We then warped these and coadded these images using the same weights
as the image data.  The off-centering and warping is important so that the PSF
...
\citep{ArmstrongCoadd}.  Note \dm\ provides a PSF coadding code, but at the
time of writing it did not perform sub-pixel offsets, so was unsuitable for our
purposes.
...
the counterfactual shear.  Using unsheared positions can result in
