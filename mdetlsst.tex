...
\newcommand{\mrj}[1]{\textcolor{blue}{[MRJ: \bf #1]}}
...
characterization or calibration of the data.  We need to interpolate problem
areas, and coadd the images, but find that the shear can be accurately
recovered if example noise images are passed through all the same processes for
use in the \mcal\ image corrections.
...
\cite{LiNofz2022}.  Unlike the simulations used in this work, these simulations
may need to reproduce specific features of the real data, such as to simulate
the properties of a representative sample of coadd cells, including the
correlated noise, masking, PSF etc.  We expect this joint calibration to be
necessary even if the shear measurement method itself is unbiased.
