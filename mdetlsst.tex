...
\citep{BoschLSST2019,BoschHSC2017}, and use a new \mdet\ code designed
specifically to work with Science Pipeline data structures and
algorithms\footnote{The software used in this work is freely available on the
internet.  URLs are provided as footnotes to the text.}.  We describe \mdet\ in
\S \ref{sec:mdet}, and the results in \S \ref{sec:results}.
...
we used noise $n$ for the final 10 year coadd.  If $N$ simulated images were
created, representing data from individual exposures to be coadded, we
re-scaled the noise appropriately, such that the noise in each image was $n *
\sqrt{N}$.

We did not simulate a realistic background and background noise for each image
(see \S \ref{sec:sim:bgerr}).  We instead used Guassian noise to approximate
...
individual images to be coadded in order to test the effect of artifacts in a
realistic way.  Rather we simulated a smaller number of images per band,
typically 1 but up to 10, with the noise scaled so that the final coadd noise
matched LSST 10 year data.  Thus, relative to expected LSST data the impact of
each artifact on the coadd was large but the aggregate number of artifacts was
small.

\subsection{Galaxies} \label{sec:sim:galaxies}

We used the \descwl\ package to generate galaxy models
\citep{DESCWLSanchez2021}.  These elliptical, color galaxy models have bulge, disk and
AGN components. This catalog has a raw density of $\sim240$ galaxies per square
arcminute, and an effective $i$-band ab magnitude limit of $\sim27$. Rather
than use the \descwl\ package to draw the models in the image, we drew the
models using Galsim in our code in order to use our own PSF models (see \S
...
\begin{figure*}
    ...
    \caption{
        ...
        and $z$ bands, but with noise corresponding to the 10 LSST coadd data.
...
shear can be any value much less than unity. We used $\gamma_j^{\pm} = \pm0.01$,
...
\begin{tikzpicture}
    ...
    % bias range as function of S/N
	\addplot+[name path=mlowvar,desert1,no markers,solid,opacity=0.5,forget plot] coordinates {
        (10.0,0.219)
        (12.5,0.078)
        (15.0,0.213)
        (20.0,0.257)
    };
    \addplot+[name path=mhighvar,desert1,no markers,solid,opacity=0.5,forget plot] coordinates {
        (10.0,1.215)
        (12.5,0.869)
        (15.0,0.973)
        (20.0,0.892)
    ...
    \addplot+[color=desert1,opacity=0.5] fill between[of=mlowvar and mhighvar];
    % \addplot+[color=gray!50,opacity=0.5] fill between[of=mlowvar and mhighvar];
    \addlegendentry{LSST 10 year PSF}

	\addplot+[name path=mlowfix,desert4,no markers,solid,opacity=0.5,forget plot] coordinates {
        (10.0,0.176295)
        (12.5,-0.00826672)
        (15.0,0.165007)
        (20.0,0.111993)
    };
    \addplot+[name path=mhighfix,desert4,no markers,solid,opacity=0.5,forget plot] coordinates {
        (10.0,1.13744)
        (12.5,0.789173)
        (15.0,0.915298)
        (20.0,0.765478)
    ...
    \addplot+[color=desert4,opacity=0.5] fill between[of=mlowfix and mhighfix];
    ...
    \addplot+[name path=Tratmlowfix,desert4,no markers,solid,opacity=0.5,forget plot] coordinates {
        (1.2, 0.165007)
        (1.3, -0.0856819)
        (1.4, -0.114263)
        (1.5, -0.370727)
    };
    \addplot+[name path=Tratmhighfix,desert4,no markers,solid,opacity=0.5,forget plot] coordinates {
    ...
    % \addplot+[color=black,pattern=vertical lines,solid] fill between[of=mlowfix and mhighfix];
    \addplot+[color=desert4,opacity=0.5] fill between[of=Tratmlowfix and Tratmhighfix];
...
\begin{figure}
    ...
    density and \snr, relative to the
...
characteristics of the original images, but could be less computationally
efficient, since the injected sources must necessarily be low density 
