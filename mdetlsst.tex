...
require further percent level corrections for blending, most likely using
simulations \citep{mdet20,LiFPFSBlending2022} .  Note such simulations will
most likely be required in any case to jointly calibrate the shear bias and the
redshift distribution of the lensed galaxies \citep{MacCrann2022,LiNofz2022}.
...
``coadded'' summed images, which typically have correlated pixel noise due to
the remapping, or ``warping'' of each image to a common frame before
combination.  For example, the imminent Rubin Observatory Legacy Survey of
Space and Time \citep[LSST,][]{IvezicLSST2008} will image each part of the
survey area hundreds of times, producing a large data volume, the processing of
which requires coaddition for all but the most efficient measurements.
\cite{ArmstrongCoadd} showed that shear measurements are possible with coadded
images in the case that object detection is perfect.
