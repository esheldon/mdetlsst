...
\esstodo{check parameters (variation factor) this was run with}
\esstodo{fill in median seeing below}

We generate spatially variable PSF models using the methods described in Appendix~A
of \citet{SheldonMdet2020} based on work by \citet{heymans2012}. \citet{heymans2012}
use images with high stellar density to fit a \vonkarman\ model of atmospheric
turbulence to the PSF variation. \citet{SheldonMdet2020} use this model, with a
modification to reduce power below 1'', to generate realizations of spatially variable
PSFs using Gaussian random fields. The final PSF model is a Moffat profile with shape
parameter $\beta=2.5$. In \citet{SheldonMdet2020} this approximate variable
PSF model was compared to detailed atmospheric and optics simulations. It's parameters
were set to generate more spatial PSF variance than one expects in a typical
Rubin exposure. The final median seeing of the generated PSFs was
\esstodo{put number here}.
...
Large, saturated stars can cause artifacts in images when they are manipulated
with \mcal- or \mdet-like algorithms. To avoid these artifacts, we use the apodization
procedure described in \citet{BeckerMdetCoadd}. For each bright star, we build a
mask that is zero in the bulk of the star and smoothly goes from zero at one near the
edge. The transition region is parameterized using the cumulative
...
\end{cases}
\end{equation}
The kernel goes from zero to one over a span of $6h$, centered on $m$.
The general logic is to set $h$ large enough that the variation across the egde is slower
than the variation given by the PSF profile, but not so large that area in the image is
wasted. We set $h$ to \esstodo{pixels} and set $m$ to the radius of the star mask hole.

\subsection{Cosmic Rays \& Bad Columns} \label{sec:sim:cosmics_badcols}

\esstodo{double check how this was all handled after the code rewrite you did}

We follow \citet{BeckerMdetCoadd} in generating our bad column and cosmic ray artifacts
For cosmic rays, We select a random location on the image, a random angle, and a random
length between 10 and 30 pixels. We then flag pixels along this line as having been hit
...
pixels are inadvertently used in the final shear estimates. We generate bad column masks
using a slightly modified Monte Carlo generator from \citet{BeckerMdetCoadd}. Each bad
column is a single pixel wide, positioned randomly on the image. We also add gaps at
random to the bad columns to simulate bad columns which do not span the full CCD.
Finally, the rates of our bad column and cosmic ray artifacts have not been tuned to
match real images beyond cursory checks that they look typical for ground based data. We of
course do not know precisely the rates of column defects in the final Rubin system. Further,
Rubin \mdet\ analyses will be done on coadds of hundreds of images, where the effects of
defects in any one image will be small in a marginal sense. More detailed testing,
ideally based on real Rubin observations, will be an avenue for future investigation.
...
\begin{figure}
	\includegraphics[width=\columnwidth]{{run-riz-drcbWsBP-cells-s2n}.pdf}
	\caption{\esstodo{caption}} \label{fig:stardens}
\end{figure}

\subsubsection{Dependence of Shear Sensitivity on Cuts}
\label{sec:results:sdensnoise}

We explored what set of cuts on stellar density and \snr\ result in the lowest
noise in the recovered shear.  In figure \ref{fig:stardenserror} we show the
uncertainty in the recovered shear as a function of maximum allowed stellar
density and \snr\ cut, relative to the cuts that provide the lowest noise.  We
find the lowest uncertainty for the case of maximum stellar density less
than 100 per
square arcminute (we did not simulate higher density) and a minimum \snr\ of
15.  For this measure of uncertainty we used only one of the paired images used
for noise cancellation.  This is because the noise cancellation is more
effective for higher \snr\ objects, resulting on smaller uncertainties for
stricter \snr\ cuts, which is not true for real data.

\begin{figure}
	\includegraphics[width=\columnwidth]{{run-riz-drcbWsBP-cells-s2n-nocancel-error}.pdf}
	\caption{\esstodo{caption}} \label{fig:stardenserror}
\end{figure}

We find that including higher stellar density areas reduces the noise, but with
limited benefit beyond a density of about 40 stars per square arcminute.
...
and $A \sim 21,000$ is the simulated area.  We find $n_\mathrm{eff} \sim 42$
