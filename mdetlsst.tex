...
We can define an accurate shear measurement method as one that is unbiased (up
to higher second order shear effects) in the presence of these types of
features, in addition to noise, objects, blending etc. that are a natural part
of astronomical images.  That is, if the input data are well characterized in
...
technique will provide an accurate shear measurement.

In actual analysis on real data, errors in the low-level characterization of
the data may contribute significantly to the systematic error.  For an accurate
shear measurement algorithm, the limiting factor would then be the
characterization of the data, not the shear measurement algorithm itself.

Another desirable property of a shear measurement method is that it is accurate
when images are combined into ``coadded'', or summed images, which, for
standard implementations, have correlated pixel noise due to the remapping, or
``warping'', of each image to a common frame before combination. LSST, for
example, will image the survey area hundreds of times, producing a large data
volume, the processing of which requires coaddition for all but the most
