...
\begin{abstract}
        ...
        produced by the \dm, and uses the pipeline algorithms when possible.
        We tested the code using a new set of simulations designed to mimic
        LSST imaging data.  The simulated images contained semi-realistic
        ...
        point spread functions.  Bright stars were saturated and simulated
        bleed trails were drawn when appropriate.  Problem areas were
        interpolated, and the images were coadded into small cells, excluding
        images not fully covering the cell to guarantee a continuous PSF.  In
        all cases the measured shear was accurate within the LSST tolerances,
        even when including images with stellar density as high as 100
        arcmin$^{-2}$.  For our analysis choices, we found an effective galaxy
...
\citep{mdet20,HoekstraMdet2021a,HoekstraMdet2021b} with constant shear.  Other
recently developed methods provide similar accuracy for isolated galaxy images
\citep{BernBFD2016, LiFPFSBlending2022} but may require further percent level
corrections from simulations to account for blending effects
...
characterized in terms of flux calibration, astrometry, background
determination, noise properties, and PSF, and if problematic features are
identified and masked, the shear code should be able to provide an accurate
shear measurement. We define an unbiased shear measurement algorithm as one
that is accurate in the presence of object detection, blending, and noise when
run on well-characterized data. Importantly, errors in these low-level
characterizations of the data may in fact contribute to the final shear
calibration for an actual data analysis. However, in this case for an unbiased
shear measurement algorithm, the limiting factor is the underlying
characterization of the data, not the shear measurement algorithm itself.
...
We simulated the calibrated exposure images produced by the \dm\ version
\texttt{w\_2021\_32}.  This data is stored in an \calexp\ data structure, which carries a
calibrated, background subtracted image, along with an estimated noise variance
image plane, world coordinate system transformation (WCS) and position
dependent PSF model.  Problem areas associated with saturation, cosmic rays,
and bad columns are marked as separate bits in an integer bit mask image plane.
...
performance of \mdet\ using well-characterized data. Future work propagating
miscalibrated data effects through \mdet\ will likely be necessary to understand
the final shear calibration in actual data analyses.
...
be unrealistically small, within two pixels, to minimize portions of the
...
atmospheric and optics simulations.
\mrb{do you mean in this work or in the mdet paper for where the parameters were set?
In the mdet paper I don't think we matched LSST.}
Its parameters were set to generate the typical spatial PSF variance expected for Rubin exposures.  The median
...
\mrb{we are making this choice in this paper for mdet. I think this should be rephrased.}
...
band, typically 1 but up to 10, with the noise scaled so that the final coadd noise matched
...
\mrb{did you end up running background removal on the coadd. Should we mention that here?}
We added a negative constant to the images to approximate errors in the
background determination.  The motivation was to test if background
estimation errors in individual images can be corrected in the final coadd.
...
the PSF at the location of the coadd center in the input image. In detail, the
coadd image center may fall at an arbitrary subpixel offset in the input image,
so as a result the PSF images were not centered on a pixel in the input images.
We then warped these PSF images and coadded them using the same weights as the
image data. The off-centering and warping is important so that the PSF includes
the same small amount of smearing present in the image interpolation. Not
including this offset results in percent level multiplicative shear biases
\citep{ArmstrongCoadd}. Note \dm\ provides a PSF coadding code, but at the time
of writing it did not perform sub-pixel offsets, so was unsuitable for our
purposes. The resulting coadd and PSF coadd data were stored in a new \calexp\
data structure for use in \mdet.
...
percent. \mrb{say we suspect this is because of non-linearity?}
...
stack. Scarlet performed better in the limited tests we performed, with pairs
of galaxies at fixed separation, but due to the high computational cost of the
deblender, we did not gather enough statistics to perform comprehensive tests.
We also did not test Scarlet in more realistic scenes with many randomly places
galaxies.
...
distribution of the lensing source galaxies.  \mrb{mention shear precision?}
We will explore deblending more in future work.

\subsection{Object Measurement} \label{sec:mdet:meas}


\mrb{the moments routines that wrap ngmix in the mdet code apply 4-fold symmetry to
zeros in the weight map. Did you do that as well? Should we mention it here?}
...
the error on the flux $\sigma(F)$, but the full covariance matrix was
measured for the moments.

For the sake of brevity, we have not shown in the sums the weight for each
pixel, which is the inverse of the background pixel noise variance (see \S
\ref{sec:sim}).  However, the background noise variance is nearly constant
across the coadd so this is only relevant for masked pixels, which were given
zero weight.  Note $T$ is an estimate of the observed square size of the
object, not the pre-PSF size.

We also record the moments of the coadded PSF for use in object selection (see
\S \ref{sec:results:full}). \mrb{this is the reconvolution PSF - might be good to say}
...
simulation features in \S \ref{sec:results:cells}. \mrb{maybe we should add a table
collating all of the results so they are easy to read/find? We can pick a single stellar density
to report at for cases where this matters.}
...
only the $i$ band with a single image ``epoch''.  We used a fixed, circular Moffat
PSF\citep{Moffat1969} with FWHM=0.8.  We applied a cut $S/N > 10$ to remove
spurious detections.  We expect a bias $m$ of a few parts in
...
any bias larger than expected from higher order shear effects.  We interpret
...
corresponding value for the PSF \mrb{reconvolution PSF?}. The lower \snr\ cut
is motivated by the noise study in \S \ref{sec:results:sdensnoise} in which a
cut at 10 or 12.5 resulted in similar shear measurement uncertainties. In all
...
crude star removal in our tests. \mrb{mention they increase noise too?}
...
test because the noise cancellation procedure works better for high \snr\
objects.  We will explore the shear sensitivity without noise cancellation in
\S \ref{sec:results:sdensnoise}.
...
the counterfactual shear. \mrb{maybe we shoudl explain what unsheared means here}
Using unsheared positions can result in
multiplicative biases of a few tenths of a percent.
...
this effect.  We may explore this more in a future work. \mrb{this might make rejoining
cells more complicated though.}
...
the inferred shear, but did increase the noise.  Due to lack of foresight, we
...
\snr\ objects have relatively small response, which is not accounted for in the
weighting.  An optimized weight based on the expected response for each object
may provide a better sensitivity \citep{GattiY3Shear2021}.  Note also that
these results will vary significantly with the measurement type, for example
the specific weight function used to measure moments, or the model that is
fitted.
...
shear recovery.  As show in \citep{BeckerMdetCoadd}, we find that the shear can be accurately recovered if
...
The third and final track is to adjust the procedures described here
in order to optimize the overall precision of the shear catalog. Possible research
directions in this vein include finding optimal object measurements to use
with \mdet\ or generalizing the work here to incorporate deep-field \mcal\
\citep{dfmcal22}. Some of this work could potentially be done directly on data
from LSST as well.

\mrb{maybe we put a punchy closing paragraph here?}
