...
galaxy images using the \mdet\ technique \citep{mdet20}.
...
may be expected to contain cosmic rays, bad CCD columns, saturation, stars
(which are not sheared) and star bleed trails.
...
identified and potentially masked, the shear algorithm should be able to
process the input data and produce accurate shear estimates.

We also expect the images may be combined into ``coadded'' aggregates, which
typically have correlated pixel noise due to the remapping, or ``warping'' to a
common pixel grid.  The imminent Rubin Observatory Legacy Survey of Space and
Time \citep[LSST,][]{IvezicLSST2008} will typically image each part of the
survey area hundreds of times, producing a large data volume, the processing of
which requires coaddition for all but the most efficient measurements.  Again,
we think the shear code should perform accurately in the presence of correlated
...
containing proxies for the above effects, which we warp and aggregate into
coadds.  We simulate the expected data products produced by the LSST data
management system \citep[\dm,][]{JuricLSST2015,BoschHSC2017}, and use a new
\mdet\ code designed specifically to work with \dmshort\ data structures and
algorithms.
