...
def do_Tratio_bias_plot(ax):
    ...
    # run-WM-nowarp
    mlows = np.array(
        [6.06497e-05, -0.000106858, -0.000248307, -0.000529498]
    )/0.001
    mhighs = np.array(
        [0.000895614, 0.000819506, 0.000831547, 0.000824817]
    )/0.001
...
def do_s2n_bias_plot(ax):
    ...
    # run-WM-nowarp
    mlows = np.array(
        [6.32742e-05, -3.74255e-05, 5.56511e-05, 3.55578e-05]
    )/0.001
    mhighs = np.array(
        [0.00111235, 0.000843535, 0.000905948, 0.000796339]
    )/0.001
    ax.fill_between(
        s2ns,
        mlows,
