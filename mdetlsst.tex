...
In terms of \snr, we find that cutting \snr\ less than 10 or 12.5 does not
result in a better \snr\ than a cut at 15, indicating that these objects are
providing relatively little information.  However, we have two notes on this.
First, we are not using any explicit weighting.   It may be that a smooth
weight that is a function of \snr, $T$, and other relevant parameters will
provide a better sensitivity \esstodo{cite Y3 paper weighting}.  Second, these
results will vary significantly with the measurement type, e.g. the weight
function or fitted model, which in turn affects the \snr\ estimate.
