...
\begin{abstract}
        ...
        characterization, or artifact identification, but we did include
        simulated background determination errors.  We turned on individual
        simulation features cumulatively in consecutive tests in order to
        isolate software bugs and possible biases.  In all cases, the measured
        shear was accurate within the LSST tolerances.  For our analysis
        choices, we found an effective galaxy density of $n_{\mathrm{eff}} \sim
        41$ per square arcminute using the $r, i$ and $z$ band images for shear
        measurement.
...
weak gravitational lensing as a primary probe to study dark energy
\citep{IvezicLSST2008,Euclid2011,Roman2015}.  In order to make full use of
these data sets, we need algorithms that produce nearly unbiased shear
estimates, and implementations of those algorithms that work with the data
products of each survey.

The \mdet\ shear measurement has been shown to accurately recover weak
gravitational shear signals from the lensed images of galaxies, up to
non-linear shear effects, in the presence of noise, a point spread function
(PSF) and overlapping, or blended, galaxy images
\citep{mdet20,HoekstraMdet2021a,HoekstraMdet2021b}.

Other recently developed methods provide similar accuracy for non-overlapping
galaxy images \citep{BernBFD2016, LiFPFSBlending2022} but are expected to
require further percent level corrections for blending
\citep{mdet20,LiFPFSBlending2022} using simulations.  Note such simulations
will most likely be required in any case to jointly calibrate the shear bias
and the redshift distribution of the lensed galaxies
\citep{MacCrann2022,LiNofz2022}.
...
determination, and PSF, and problematic features are identified and potentially
masked, the shear code should be able to further process the images to provide
an accurate shear measurement.

The shear measurement should also be accurate when images are combined into
``coadded'' summed images, which typically have correlated pixel noise due to the
remapping, or ``warping'' of each image to a common frame before combination.
The imminent Rubin Observatory Legacy Survey of Space and Time
\citep[LSST,][]{IvezicLSST2008} will typically image each part of the survey
area hundreds of times, producing a large data volume, the processing of which
requires coaddition for all but the most efficient measurements.
\cite{ArmstrongCoadd} showed that shear measurements are possible with
coadded images in the case that object detection is perfect.

At the time of writing, we consider lower level characterizations of the data,
such as determining the flux and astrometric calibration, PSF characterization,
background determination and finding of artifacts, to be outside the purview of
the shear measurement code.  We expect these tasks to be performed by the
``upstream'' software pipeline.
...
the weighting.  An optimized weight based on the expected response for each
object may provide a better sensitivity \citep{GattiY3Shear2021}.  Note also
that these results will vary significantly with the measurement type, for
example the specific weight function used to measure moments, or the model that
is fitted.
...
We have shown \mdet\ to be robust to the types of image features we expect the
shear code to handle, namely those that do not require further characterization
or calibration of the data.  We need to interpolate problem areas, and coadd
the images, but find that the shear can be accurately recovered if example
noise images are passed through all the same processes for use in the \mcal\
image corrections.

We expect further simulation work will be critical moving forward.  We plan to
proceed on two separate tracks.  The first track is to generate realistic
simulations to jointly calibrate the redshift dependent shear bias and redshift
distribution $N(z)$ of the sources, following the pioneering work of
\cite{MacCrann2022} and \cite{LiNofz2022}.  We expect this joint calibration to
be necessary even if the shear measurement method itself is unbiased.
...
the noise properties are misestimated, or the data are miscalibrated in some
other way.  It may be that relatively small, targeted simulations are
sufficient to limit these effects.
