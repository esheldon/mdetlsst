...
We used a two part weight function, one part based on the shape noise
and a second part to downweight a particular set of spurious objects.

The primary weight was based on the intrinsice shape noise and shape
measurement uncertainty
\begin{equation}
    W_\sigma = \frac{1}{\sigma_{SN}^2 + \sigma_{\epsilon}^2}
\end{equation}
\esstodo{cite}
where $\sigma_{SN}$ is the intrinsic ellipticity noise of the object
and $\sigma_{epsilon}$ is the estimated uncertainty.  We find $\sigma_{SN}$
before response correction is about 0.07.  The average response is about
0.26 for this sample, implying the true shape noise is about 0.27.

In the presence of stars we found a population of high ellipticity objects.
These objects were usually spurious detections or blends of faint objects with
bright stars, found just outside our star masking radius\footnote{Masaya
Yamamoto, private communication}.  We did not keep enough information to
exclude these objects from our catalogs after the fact, and rerunning was
prohibitive due to limited computing resources.  As a temporary solution we
included an additional ellipticity dependent weight function \citep{ba14}
\begin{equation}
    W_g(|g|) = \left[1 - g^2\right]^2 \mathrm{exp}\left[ -e^2/2 \sigma^2\right]
\end{equation}
with $\sigma = 0.3$, which downweights very high ellipticity objects.  We found
that using this weight function reduced the uncertainty of the shear recovery
by 13\% and removed a small ($m ~ 0.001$) bias at low S/N.  In future work it
will be important to use sufficient star masking and identify any problematic
unmasked objects.

The total weight function was
\begin{equation}
    W = W_\sigma \times W_g
\end{equation}
...
in addition to a cut on size ratio $T/T_{PSF} > 1.2$. \esstodo{describe results}
...
We explored what set of cuts on stellar density and \snr\ resulted in the
lowest relative noise in the recovered shear.  In order to get an accurate
measure of the relative uncertainty, we did not cancel noise using the results
from paired images.  The noise cancellation is most effective for higher \snr\
objects that are more likely to be detected in both of the paired images, which
would results in artificially smaller uncertainties for higher minimum \snr\
cuts.
...
dependence on stellar density is fairly flat for stellar density above 40 per
square arcminute.  Including higher stellar density areas reduces the noise,
but with limited benefit beyond a density of about 40 stars per square
arcminute.
...
shear uncertainty compared to cutting at 12.5.  It may seem surprising that
including more objects resulted in a noisier measurement.  This can occur with
\mcal\ because the shear estimator involves dividing by the mean response,
which is typically smaller and noisier for lower \snr\ objects.  However, we
have two notes on this.  First, it may be that an optimized weight that is a
function of \snr, $T$, and other relevant parameters will provide a better
sensitivity \esstodo{cite Y3 paper weighting}.  Second, these results will vary
significantly with the measurement type, for example the specific weight
function used to measure moments, or the model that is fitted, each of which
will provide a different estimate of the \snr.
...
41,600 square degrees of images, with about 6\% of that area masked due to
bright stars and defects.  We cut to a maximum stellar density of 60 per square
arcminute, and remove fields with overall masking greater than 10\%, which
leaves a final area of $A \sim 32,100$ square degrees.  We converted the measured
variance to an effective density using a simple formula
\begin{equation} \label{eq:neff}
    n_{\mathrm{eff}} = \frac{\sigma^2_{e}}{R^2 \mathrm{A} \sigma^2_{\gamma}}.
\end{equation}
In equation \ref{eq:neff}, $R \sim 0.242$ is the scalar response from averaging
the diagonal components of the response matrix, $\sigma_{SN} \sim 0.098$ is the
total, weighted ellipticity standard deviation for our catalog after cuts and
$\sigma_{\gamma} \sim 5.88 \times 10^{-6}$ is the $1-\sigma$ uncertainty in the
recovered shear without noise cancellation.  We find $n_\mathrm{eff} \sim 41$
per square arcminute for the basic cuts (see \S \ref{sec:results:more}).
The effective density will vary with the choice of ellipticity estimate, and
may be improved with optimized weights.
